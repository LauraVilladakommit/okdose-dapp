...
const CONTRACT_ACCOUNT = process.env.MILTEFOSINE_CONTRACT as string;
const BLOCK_ID = process.env.MILTEFOSINE_BLOCK_ID as string;
...
test('It returns the correct type attributes', async (t) => {
  ...
  const dose: any = await contract.view(CONTRACT_METHOD, { weight: 70 });
