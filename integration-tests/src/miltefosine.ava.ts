...
const CONTRACT_ACCOUNT = process.env.MILTEFOSINE_CONTRACT;

const BLOCK_ID = process.env.MILTEFOSINE_BLOCK_ID;
...
test('It returns the correct type attributes', async (t) => {
  ...
  const dose: any = await contract.view('get_dose', { weight: 60 });
